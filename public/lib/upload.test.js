...
describe("uploading", () => {
    ...
    test('upload the example image', async () => {
        Wallpaper.find({}, function(err, allWallpapers) {
            let len = allWallpapers.length;
            document.getElementById('test1').innerHTML = len;
            upload('https://images.unsplash.com/photo-1518020382113-a7e8fc38eac9?ixlib=rb-0.3.5&ixid=eyJhcHBfaWQiOjEyMDd9&s=71d59cd22de21da8d2939bc203617983&auto=format&fit=crop&w=1320&q=80');  
            Wallpaper.find({}, function(err, allWallpapers){
                if(err) {
                    console.log(err);
                } else {
